...
| [`NTFS/`](NTFS) | NTFS permission auditing/reporting | 2 |
...
[`NTFS/Export-NtfsPermissionReport.ps1`](NTFS/Export-NtfsPermissionReport.ps1) scans a folder tree, resolves every ACL entry against AD (with optional nested-group expansion), and produces CSV/JSON plus a clickable Explorer-style HTML report. Run it with no arguments to get a picker window instead of passing switches on the command line.

A live example of the HTML report — built from fake sample data, never a real scan — is at:

**[get-nout.github.io/Powershell/NTFS/demo-report.html](https://get-nout.github.io/Powershell/NTFS/demo-report.html)**

It's generated by [`NTFS/New-DemoReport.ps1`](NTFS/New-DemoReport.ps1), which pulls the actual rendering code from the real script so the demo can't drift out of sync with what it really produces.
